...
const urlPatterns = [
  ...
  // Service pages
  {
    en: "/en/services/executive-english/",
    es: "/es/servicios/ingles-para-ejecutivos/",
  },
  {
    en: "/en/services/startup-founders/",
    es: "/es/servicios/ingles-para-fundadores-de-startups/",
  },
  {
    en: "/en/services/tech-english/",
    es: "/es/servicios/ingles-para-tecnologia/",
  },
  {
    en: "/en/services/logistics-english/",
    es: "/es/servicios/ingles-para-logistica/",
  },
  {
    en: "/en/services/professional-english/",
    es: "/es/servicios/ingles-para-profesionales/",
  },
  {
    en: "/en/services/high-stakes-english/",
    es: "/es/servicios/ingles-para-presentaciones/",
  },
  ...
  // Categories
  {
    en: "/en/category/business-english/",
    es: "/es/category/business-english/",
  },
  {
    en: "/en/category/high-impact-communication/",
    es: "/es/category/high-impact-communication/",
  },
  {
    en: "/en/category/career-leadership/",
    es: "/es/category/career-leadership/",
  },
  {
    en: "/en/category/english-coaching/",
    es: "/es/category/english-coaching/",
  },
  {
    en: "/en/category/executive-english/",
    es: "/es/category/executive-english/",
  },
...
async function auditUrlPatterns() {
  ...
  console.log(
    `${colors.magenta}${colors.bold}[1] AUDITING URL PATTERNS${colors.reset}`,
  );
  console.log(
    `${colors.blue}Checking symmetry between English and Spanish URLs...${colors.reset}\n`,
  );
  ...
  for (const pattern of urlPatterns) {
      ...
      // For category pages
        ...
        try {
          ...
          if (!hasEnCat || !hasEsCat) {
            console.log(
              `${colors.yellow}⚠️ CATEGORY DATA CHECK: ${colors.reset}Category ${catSlug}/${esCatSlug} may be missing in categories.ts`,
            );
          }
        } catch (err) {
          console.log(
            `${colors.yellow}⚠️ CATEGORY CHECK: ${colors.reset}Could not verify category in data file`,
          );
        }
      }
      // For service pages
        ...
        if (
          await fileExists(path.join(projectRoot, `src/pages/${enPath}.astro`))
        ) {
    ...
    if (enExists && esExists) {
      console.log(
        `${colors.green}✓ SYMMETRICAL: ${colors.reset}${pattern.en} ↔ ${pattern.es}`,
      );
      success++;
      ...
      if (enPath.endsWith(".md") && esPath.endsWith(".md")) {
        try {
          const enContent = await fs.readFile(
            path.join(projectRoot, enPath),
            "utf-8",
          );
          const esContent = await fs.readFile(
            path.join(projectRoot, esPath),
            "utf-8",
          );

          const hasEnTranslation =
            enContent.includes(`es: "${pattern.es}"`) ||
            enContent.includes(`es: '${pattern.es}'`);
          const hasEsTranslation =
            esContent.includes(`en: "${pattern.en}"`) ||
            esContent.includes(`en: '${pattern.en}'`);

          if (!hasEnTranslation || !hasEsTranslation) {
            console.log(
              `  ${colors.yellow}⚠️ TRANSLATION LINKS: ${colors.reset}Files exist but may have incorrect translation links`,
            );
          }
        } catch (err) {
      ...
      if (!enExists) {
        console.log(
          `${colors.red}❌ MISSING EN: ${colors.reset}${pattern.en} (${enPath} not found)`,
        );
        issues++;
      }
      if (!esExists) {
        console.log(
          `${colors.red}❌ MISSING ES: ${colors.reset}${pattern.es} (${esPath} not found)`,
        );
        issues++;
      }
    }
...
async function checkCommonPatterns() {
  ...
  try {
    ...
    for (const file of layoutFiles) {
      ...
      if (
        content.includes("language-switcher") ||
        content.includes("languageSwitcher")
      ) {
        console.log(
          `${colors.green}✓ LANGUAGE SWITCHER: ${colors.reset}Found in ${path.basename(file)}`,
        );
    ...
    if (!hasLangSwitcher) {
      console.log(
        `${colors.yellow}⚠️ LANGUAGE SWITCHER: ${colors.reset}Could not find language switcher component in layout files`,
      );
    }
  } catch (err) {
    console.log(
      `${colors.yellow}⚠️ LAYOUT CHECK: ${colors.reset}Could not verify layouts`,
    );
  }
}
...
function printSummary(issues, success) {
  ...
  if (issues === 0) {
    console.log(
      `${colors.green}${colors.bold}✓ PERFECT SYMMETRY! ${colors.reset}All ${total} URL patterns have proper bilingual versions.`,
    );
  } else {
    console.log(
      `${colors.yellow}Found ${issues} URL pattern issues out of ${total} checked (${percentage}% complete).${colors.reset}`,
    );
    console.log(
      `${colors.green}Successfully verified: ${success} URL patterns${colors.reset}`,
    );
    console.log(
      `${colors.yellow}Issues found: ${issues} URL patterns${colors.reset}`,
    );
