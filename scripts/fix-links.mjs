...
const replacements = [
    ...
    { pattern: /href="\/en\/testimonials"(?!\/)/g, replace: 'href="/en/testimonials/"' },
    ...
    { pattern: /href="\/es\/servicios"(?!\/)/g, replace: 'href="/es/servicios/"' },
    { pattern: /href="\/es\/testimonios"(?!\/)/g, replace: 'href="/es/testimonios/"' },
    { pattern: /href="\/es\/contact"(?!\/)/g, replace: 'href="/es/contact/"' },

    // --- Legal pages (exact) ---
    { pattern: /href="\/en\/legal\/privacy-policy"(?!\/)/g, replace: 'href="/en/legal/privacy-policy/"' },
    { pattern: /href="\/en\/legal\/terms-of-service"(?!\/)/g, replace: 'href="/en/legal/terms-of-service/"' },
    { pattern: /href="\/es\/legal\/privacy-policy"(?!\/)/g, replace: 'href="/es/legal/privacy-policy/"' },
    { pattern: /href="\/es\/legal\/terms-of-service"(?!\/)/g, replace: 'href="/es/legal/terms-of-service/"' },

    // --- Categories: ensure /…/slug/ ---
    { pattern: /href="\/en\/category\/([a-z0-9-]+)(?=")/g, replace: 'href="/en/category/$1/"' },
    { pattern: /href="\/es\/category\/([a-z0-9-]+)(?=")/g, replace: 'href="/es/category/$1/"' },

    // --- Blog pagination (no-slashed -> slashed) ---
    { pattern: /href="\/(en|es)\/blog\/([0-9]+)(?=")/g, replace: 'href="/$1/blog/$2/"' },

    // --- Kill /es/blog/blog/N duplicates ---
    { pattern: /href="\/es\/blog\/blog\/([0-9]+)\/?(?=")/g, replace: 'href="/es/blog/$1/"' },

    // --- Temporary: service detail ghosts -> index (EN) ---
    { 
        pattern: /href="\/en\/services\/(executive-english|startup-founders|tech-english|logistics-english|professional-english|high-stakes-english)\/?(?=")/g,
        replace: 'href="/en/services/"'
    },

    // --- Temporary: stray /es/services/... -> /es/servicios/ (ES root is "servicios") ---
    { pattern: /href="\/es\/services\/[^"]+\/?(?=")/g, replace: 'href="/es/servicios/"' },
    ...
    { pattern: /https:\/\/www\.www\.nyenglishteacher\.com/g, replace: 'https://www.nyenglishteacher.com' },
    { pattern: /new-york-english-og-wide\.jpg/g, replace: 'new-york-english-og.jpg' }
];
...
async function processFile(filePath) {
    try {
        ...
        console.warn(`Skipping unreadable file: ${path.relative(projectRoot, filePath)} - ${error.message}`);
        return false;
    }
}
