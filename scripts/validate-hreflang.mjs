...
function checkMissingMappings() {
  ...
  for (const page of allPages) {
    ...
    if (!hreflangMappings[normalized] && !page.includes('[') && !page.includes('...')) {
      validationCount++;
      addIssue(
        'missing_mapping',
        'Medium',
...
async function writeJsonReport(startTime) {
  const report = {
    ...
    summary: {
      ...
      warnings: issues.filter(i => ['Low', 'Medium'].includes(i.priority)).length,
      errors: issues.filter(i => ['High', 'Critical'].includes(i.priority)).length
    },
    issues,
    metadata: {
...
function generateConsoleReport() {
  ...
  console.log('\n💡 Recommendations:');
  console.log('1. Fix all critical errors to ensure proper hreflang implementation');
  console.log('2. Review high priority issues for potential SEO improvements');
  console.log('3. Run this validator after making changes to hreflang mappings');
...
async function main() {
  ...
  try {
    ...
    const criticalErrors = issues.filter(i => i.priority === 'Critical').length;
