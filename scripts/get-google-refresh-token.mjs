...
if (!process.env.GOOGLE_CLIENT_ID || !process.env.GOOGLE_CLIENT_SECRET || !process.env.GOOGLE_REDIRECT_URI) {
  ...
  console.error('  GOOGLE_REDIRECT_URI="http://localhost:4321/api/auth/callback"');
  process.exit(1);
}
...
rl.question('Enter the authorization code: ', async (code) => {
  try {
    ...
    if (tokens.expiry_date) {
      ...
      console.log('💡 Don\'t worry - the refresh token will automatically get new access tokens!\n');
    }
