...
function classifyWarnings(files) {
    ...
    const rootReports = files.filter(f =>
        f.match(/\.(log|csv)$/i) && path.dirname(f) === ""
    );
    ...
    const rootScripts = files.filter(f =>
        f.match(/\.(ps1|mjs|js|patch)$/i) && path.dirname(f) === ""
    );
...
async function main() {
    ...
    console.log(`📊 Found ${issues.length} issues: ${issues.filter(i => i.severity === "red").length} critical, ${issues.filter(i => i.severity === "orange").length} medium, ${issues.filter(i => i.severity === "yellow").length} low`);
}
