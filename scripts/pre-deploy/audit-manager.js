...
class AuditManager {
  ...
  generateFinalReport() {
    ...
    const implementedResults = Object.entries(this.results).filter(
      ([_, status]) => status !== null,
    );

    const allPassed = implementedResults.every(
      ([_, status]) => status === "PASS",
    );
    const passedCount = implementedResults.filter(
      ([_, status]) => status === "PASS",
    ).length;
