...
class FullSiteScan {
  ...
  scanFile(filePath) {
    ...
    if (
      ...
      description.length < SEO_RULES.description.min
    ) {
  ...
  generateReport() {
    ...
    if (this.issues.descriptionTooShort.length > 0) {
      ...
      this.issues.descriptionTooShort.forEach((issue) => {
        ...
        console.log(
          `    └─ ${issue.length} chars (min ${SEO_RULES.description.min})`,
        );
    ...
    if (this.issues.descriptionTooLong.length > 0) {
      ...
      this.issues.descriptionTooLong.forEach((issue) => {
        console.log(`  ${issue.url}`);
        console.log(
          `    └─ ${issue.length} chars (max ${SEO_RULES.description.max})`,
        );
        console.log(`    └─ "${issue.description}"\n`);
      });
    }
    ...
    if (this.issues.titleTooLong.length > 0) {
      ...
      this.issues.titleTooLong.forEach((issue) => {
        console.log(`  ${issue.url}`);
        console.log(
          `    └─ ${issue.length} chars (max ${SEO_RULES.title.max})`,
        );
    ...
    if (this.issues.titleTooShort.length > 0) {
      ...
      this.issues.titleTooShort.forEach((issue) => {
        console.log(`  ${issue.url}`);
        console.log(
          `    └─ ${issue.length} chars (min ${SEO_RULES.title.min})`,
        );
        console.log(`    └─ "${issue.title}"\n`);
      });
    }
    ...
    console.log(
      `  • Meta descriptions too short: ${this.issues.descriptionTooShort.length}`,
    );
    console.log(
      `  • Meta descriptions too long: ${this.issues.descriptionTooLong.length}`,
    );
