...
function validateRedirectCoverage() {
  ...
  console.log(`📈 Redirect coverage: ${coveredPatterns}/${commonPatterns.length} common patterns covered`);
}
      ...
      if (line && !line.startsWith('#')) {
        ...
        if (parts.length >= 3) {
          ...
          if (!from.startsWith('/') && !from.startsWith('http')) {
            warnings.push(`⚠️  Line ${i + 1}: Redirect source should start with / or http: ${from}`);
          }
...
function checkCommonPatterns() {
  ...
  for (const pattern of commonPatterns) {
    console.log(`📄 Common file: ${pattern} (should exist in public/ or have redirect)`);
  }
}
