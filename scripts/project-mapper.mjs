...
class ProjectMapper {
  ...
  parseI18nFile() {
    ...
    try {
      ...
      if (routeMatch) {
        ...
        const enMatch = routeContent.match(/en:\s*{([^}]+(?:{[^}]*}[^}]*)*?)}/s);
        if (enMatch) {
          const enRoutes = enMatch[1];
          const routeEntries = enRoutes.match(/["']([^"']+)["']:\s*["']([^"']+)["']/g);
          if (routeEntries) {
            routeEntries.forEach(entry => {
              const [, tkey, route] = entry.match(/["']([^"']+)["']:\s*["']([^"']+)["']/);
        ...
        const esMatch = routeContent.match(/es:\s*{([^}]+(?:{[^}]*}[^}]*)*?)}/s);
        if (esMatch) {
          const esRoutes = esMatch[1];
          const routeEntries = esRoutes.match(/["']([^"']+)["']:\s*["']([^"']+)["']/g);
          if (routeEntries) {
            routeEntries.forEach(entry => {
              const [, tkey, route] = entry.match(/["']([^"']+)["']:\s*["']([^"']+)["']/);
      ...
      console.log(`✅ Found ${Object.keys(this.analysis.routeMappings.en).length} English routes`);
      console.log(`✅ Found ${Object.keys(this.analysis.routeMappings.es).length} Spanish routes`);
  ...
  scanPageFiles(dir = './src/pages', basePath = '') {
    try {
      ...
      for (const item of items) {
        ...
        if (stat.isDirectory() && !item.startsWith('_') && !item.includes('[')) {
  ...
  analyzePageContent(filePath) {
    try {
      ...
      const analysis = {
        usesBase: content.includes('import Base from') && content.includes('@layouts/Base.astro'),
        lang: null,
        tkey: null,
        title: null,
        description: null
  ...
  performAnalysis() {
    ...
    const checkMissingFiles = (lang, routes) => {
      for (const [tkey, route] of Object.entries(routes)) {
        const expectedFile = this.analysis.actualFiles.find(f => 
          f.route === route && f.lang === lang
        );
    ...
    for (const file of this.analysis.actualFiles) {
      const hasMapping = Object.values(this.analysis.routeMappings.en).includes(file.route) ||
  ...
  routeToFilePath(route) {
    ...
    if (route === '/' || route === '/en/' || route === '/es/') {
      const lang = route.includes('/en/') ? 'en' : route.includes('/es/') ? 'es' : 'en';
  ...
  generateReport() {
    ...
    writeFileSync('./project-reports/project-mapping-analysis.json', JSON.stringify(report, null, 2));
