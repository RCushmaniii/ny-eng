...
try {
  ...
  console.error('Code being evaluated:', mappingsCode.substring(0, 200) + '...');
  process.exit(1);
}
...
function validateReciprocalLinks() {
  ...
  for (const [path, mapping] of Object.entries(hreflangMappings)) {
    ...
    if (hreflangMappings[en] && hreflangMappings[en].es !== es) {
      errors.push(`❌ Non-reciprocal link: ${en} -> ${hreflangMappings[en].es} but ${path} -> ${es}`);
    }
    
    // Check if Spanish page has reciprocal English link
    if (hreflangMappings[es] && hreflangMappings[es].en !== en) {
      errors.push(`❌ Non-reciprocal link: ${es} -> ${hreflangMappings[es].en} but ${path} -> ${en}`);
    }
  }
}
...
function checkMissingMappings() {
  ...
  for (const page of allPages) {
    ...
    if (!hreflangMappings[normalizedPage] && !page.includes('[') && !page.includes('...')) {
