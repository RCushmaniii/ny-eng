...
async function auditStaticPages() {
    ...
    try {
        ...
        try {
            ...
            console.log(`${colors.yellow}Static page directories not found. Skipping this phase.${colors.reset}`);
            return 0;
        }
        ...
        if (asymmetryCount === 0) {
            ...
            console.log(`${colors.yellow}ACTION NEEDED: Create missing Spanish .astro pages.${colors.reset}`);
        }
...
function findMatchingFileFromTranslation(translationPath, allFiles) {
    ...
    for (const file of allFiles) {
        ...
        const fileRelative = file.replace(/^.*\/blog\/(en|es)\//, '').replace(/^.*\/legal\/(en|es)\//, '');
...
async function auditContentCollections() {
    ...
    try {
        ...
        try {
            ...
            console.log(`${colors.yellow}Content directory not found. Skipping this phase.${colors.reset}`);
            return 0;
        }
        ...
        const collectionDirs = collections
            ...
            .filter(name => !excludePatterns.some(pattern => name.includes(pattern.replace('**/', '').replace('/**', ''))));
        ...
        for (const collection of collectionDirs) {
            ...
            for (const file of mdFiles) {
                ...
                try {
                    ...
                    try {
                        ...
                        translationStatus.set(relativePath, { status: 'frontmatter-parse-error', error: err.message });
                        continue;
                    }

                    if (frontmatterObj && frontmatterObj.translations && Object.keys(frontmatterObj.translations).length > 0) {
            ...
            for (const [filePath, status] of translationStatus.entries()) {
                if (status.status === 'has-translations') {
                    for (const [lang, targetPath] of Object.entries(status.translations)) {
                        // Normalize paths for comparison
                        const normalizedTargetPath = targetPath.replace(/^\/?/, '/');

                        // Save to translation pairs
                        const pairKey = lang === 'en' ? `${normalizedTargetPath}:${filePath}` : `${filePath}:${normalizedTargetPath}`;
                        translationPairs.set(pairKey, { [lang]: filePath, target: targetPath });
                    }
                }
            }
            ...
            if (filesWithoutFrontmatter.length > 0) {
                ...
                filesWithoutFrontmatter.forEach(file => {
                    console.log(`${colors.yellow}⚠️ NO FRONTMATTER: ${colors.reset}${file}`);
                });
            ...
            const filesWithoutTranslations = [...translationStatus.entries()]
                .filter(([_, status]) => status.status === 'no-translations' || status.status === 'empty-translations')
                .map(([file, _]) => file);

            if (filesWithoutTranslations.length > 0) {
                const notTemplate = filesWithoutTranslations.filter(file =>
            ...
            for (const [filePath, status] of translationStatus.entries()) {
                ...
                const fileLang = filePath.includes('/en/') ? 'en'
                    : filePath.includes('/es/') ? 'es'
                        : Object.keys(status.translations).find(l => l === 'en' || l === 'es') || 'en';
                ...
                const cleanThisUrl = (thisUrl || '').replace(/^\/+|\/+$/g, '').toLowerCase();
                const cleanTargetBackUrl = (targetBackUrl || '').replace(/^\/+|\/+$/g, '').toLowerCase();
                ...
                brokenPairs.forEach(pair => {
                    switch (pair.issue) {
                        case 'missing-translation-link':
                            console.log(`${colors.red}❌ MISSING TRANSLATION: ${colors.reset}${pair.source} (no ${pair.lang} translation link in frontmatter)`);
                            break;
                        case 'target-not-found':
                            console.log(`${colors.red}❌ MISSING TARGET: ${colors.reset}${pair.source} → ${pair.target} (target file not found)`);
                            break;
                        case 'no-reciprocal':
                            console.log(`${colors.red}❌ NO RECIPROCAL: ${colors.reset}${pair.source} → ${pair.target} (target has no translations)`);
                            break;
                        case 'incorrect-reciprocal':
                            console.log(`${colors.red}❌ MISMATCHED LINK: ${colors.reset}${pair.source} → ${pair.target} (points to ${pair.targetPath} instead of ${pair.sourcePath})`);
                            break;
                    }
                });
                ...
                brokenPairs.forEach(pair => {
                    switch (pair.issue) {
                        ...
                        case 'missing-reciprocal':
                            console.log(`${colors.red}❌ MISSING BACK-LINK: ${colors.reset}${pair.source} → ${pair.target} (target missing ${pair.lang === 'en' ? 'es' : 'en'} translation)`);
                            break;
                        case 'incorrect-reciprocal':
                            console.log(`${colors.red}❌ MISMATCHED LINK: ${colors.reset}${pair.source} → ${pair.target} (points to ${pair.targetPath} instead of ${pair.sourcePath})`);
                            break;
...
async function auditDataFiles() {
    ...
    try {
        ...
        try {
            ...
            console.log(`${colors.yellow}Data directory not found. Skipping this phase.${colors.reset}`);
            return 0;
        }
...
function printSummary(staticIssues, contentIssues, dataIssues) {
    ...
    if (totalIssues === 0) {
        console.log(`${colors.green}${colors.bold}✓ PERFECT SYMMETRY! ${colors.reset}Your site is fully bilingual and ready for sitemap generation.`);
    } else {
