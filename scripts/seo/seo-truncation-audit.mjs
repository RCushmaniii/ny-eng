...
class SEOAuditor {
  ...
  checkLayoutUsage(content, filePath) {
    if (
      content.includes("import Layout from") &&
      content.includes("@layouts/Layout.astro")
    ) {
      this.issues.push({
    ...
    if (
      content.includes("translationSlugEn") ||
      content.includes("translationSlugEs")
    ) {
      this.issues.push({
        type: "LEGACY_TRANSLATION_PROPS",
        file: filePath,
        message:
          "Still using legacy translation props instead of centralized i18n system",
      });
    }
  ...
  generateReport() {
    ...
    const oldLayoutCount = this.issues.filter(
      (i) => i.type === "OLD_LAYOUT_USAGE",
    ).length;
    const legacyPropsCount = this.issues.filter(
      (i) => i.type === "LEGACY_TRANSLATION_PROPS",
    ).length;
    ...
    if (legacyPropsCount > 0) {
      console.log(
        "3. Remove legacy translation props and use centralized i18n",
      );
    }
    if (this.warnings.length > 0) {
      console.log(
        "4. Review long titles/descriptions for optimization (optional)",
      );
    }
    if (
      !this.truncationFound &&
      oldLayoutCount === 0 &&
      legacyPropsCount === 0
    ) {
