...
## Structured Data & SEO
- All Astro pages must include a `<script type="application/ld+json">` block for organization structured data, using `{JSON.stringify(obj)}`.
- Do **not** use Astro directives like `set:head` for static JSON-LD output. Use plain HTML `<script>` in the `<head>`.
- SEO meta tags (`<title>`, `<meta name="description">`) must be set per-page using imported constants. **Do not hardcode** titles or descriptions inline.
