...
export function validateQuizConfig(config: QuizConfig): void {
  ...
  (["en", "es"] as const).forEach((lang) => {
    ...
    if (!langConfig.results.tiers) {
      throw new Error(
        `[${config.quizId}][${lang}] Missing 'results.tiers' object.`,
      );
    }
    ...
    requiredTiers.forEach((tierKey) => {
      ...
      if (!tier) {
        throw new Error(
          `[${config.quizId}][${lang}] Missing result tier: "${tierKey}"`,
        );
      }
      if (!tier.title || !tier.description) {
        throw new Error(
          `[${config.quizId}][${lang}] Tier "${tierKey}" is missing title or description`,
        );
      }
    });
    ...
    if (
      !langConfig.results.eliteComparison ||
      !Array.isArray(langConfig.results.eliteComparison.items)
    ) {
      throw new Error(
        `[${config.quizId}][${lang}] Missing or invalid 'results.eliteComparison'`,
      );
    }
    ...
    if (!langConfig.results.cta || !langConfig.results.cta.title) {
      throw new Error(
        `[${config.quizId}][${lang}] Missing 'results.cta' or cta title`,
      );
    }
    ...
    if (!langConfig.title || langConfig.title.trim() === "") {
      throw new Error(`[${config.quizId}][${lang}] Quiz title is empty`);
    }

    if (!langConfig.subtitle || langConfig.subtitle.trim() === "") {
      throw new Error(`[${config.quizId}][${lang}] Quiz subtitle is empty`);
    }
  });
