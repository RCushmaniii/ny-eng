        ...
        try {
            ...
            for (const file of files) {
                ...
                if (match && match[1]) {
                    ...
                    if (hasTranslationKey || hasTranslations || hasSpanishLink || hasEnglishLink) {
...
async function auditSitemap() {
    ...
    console.log(`${colors.blue}Starting comprehensive sitemap audit...${colors.reset}\n`);
...
async function auditStaticPages() {
    ...
    console.log(`${colors.blue}Checking symmetry between English and Spanish .astro pages...${colors.reset}\n`);
    ...
    try {
        ...
        excludePatterns.forEach(pattern => {
            enPages = enPages.filter(file => !file.includes(pattern.replace('**/', '').replace('/**', '')));
        });
        ...
        for (const enPage of enPages) {
            ...
            try {
                ...
                console.log(`${colors.green}✓ SYMMETRICAL: ${colors.reset}${relativePath}`);
            } catch (error) {
                console.log(`${colors.red}❌ ASYMMETRICAL: ${colors.reset}${relativePath} (Missing Spanish version)`);
                asymmetryCount++;
            }
        }
        ...
        if (asymmetryCount === 0) {
            ...
            console.log(`${colors.yellow}ACTION NEEDED: Create matching Spanish pages or verify these pages should be English-only.${colors.reset}`);
        }
    } catch (error) {
        console.error(`${colors.red}Error auditing static pages:${colors.reset}`, error);
    }
}
...
async function auditContentCollections() {
    ...
    console.log(`${colors.blue}Checking translation links in Markdown content...${colors.reset}\n`);
    ...
    try {
        ...
        const collectionDirs = collections.filter(dirent => dirent.isDirectory()).map(dirent => dirent.name);
        ...
        for (const collection of collectionDirs) {
            ...
            if (mdFiles.length === 0) {
                console.log(`  ${colors.yellow}No markdown files found in collection.${colors.reset}`);
                continue;
            }
            ...
            for (const file of mdFiles) {
                try {
                    ...
                    if (!frontmatterMatch) {
                        console.log(`${colors.yellow}⚠️ NO FRONTMATTER: ${colors.reset}${relativePath}`);
                        continue;
                    }
                    ...
                    const translationsMatch = frontmatter.match(/translations:\s*{([^}]*)}/);
                    if (translationsMatch) {
                        ...
                        if (enMatch) translationLinks.set(relativePath, { type: 'translations', lang: 'en', target: enMatch[1] });
                        if (esMatch) translationLinks.set(relativePath, { type: 'translations', lang: 'es', target: esMatch[1] });
                    }

                    // Format 2: translationKey
                    const translationKeyMatch = frontmatter.match(/translationKey:\s*['"]([^'"]*)['"]/);
                    if (translationKeyMatch) {
                        translationLinks.set(relativePath, { type: 'translationKey', key: translationKeyMatch[1] });
                    }

                    // Format 3: direct links
                    const spanishLinkMatch = frontmatter.match(/(?:spanish_link|spanishVersion):\s*['"]([^'"]*)['"]/);
                    const englishLinkMatch = frontmatter.match(/(?:english_link|englishVersion):\s*['"]([^'"]*)['"]/);

                    if (spanishLinkMatch) translationLinks.set(relativePath, { type: 'direct', lang: 'es', target: spanishLinkMatch[1] });
                    if (englishLinkMatch) translationLinks.set(relativePath, { type: 'direct', lang: 'en', target: englishLinkMatch[1] });

                } catch (error) {
                    console.error(`${colors.red}Error reading file ${file}:${colors.reset}`, error);
                }
            }
            ...
            for (const [filePath, linkInfo] of translationLinks.entries()) {
                try {
                    if (linkInfo.type === 'translations' || linkInfo.type === 'direct') {
                        ...
                        if (!addressBook.has(targetPath) && !addressBook.has(`src/${targetPath}`)) {
                            console.log(`${colors.red}❌ BROKEN LINK: ${colors.reset}${filePath} points to non-existent ${linkInfo.lang} file: ${targetPath}`);
                            collectionIssues++;
                            continue;
                        }

                        // Get full path of target file
                        const targetFullPath = addressBook.has(targetPath) ? targetPath : `src/${targetPath}`;

                        // Check if target file points back
                        const targetLinkInfo = translationLinks.get(targetFullPath);
                        if (!targetLinkInfo) {
                            console.log(`${colors.red}❌ NON-RECIPROCAL: ${colors.reset}${filePath} points to ${targetFullPath}, but target has no translation link`);
                            collectionIssues++;
                        } else if (targetLinkInfo.type === 'translations' || targetLinkInfo.type === 'direct') {
                            // Check if target points back to source
                            if (targetLinkInfo.target !== filePath && targetLinkInfo.target !== filePath.replace(/^src\//, '')) {
                                console.log(`${colors.red}❌ MISMATCHED LINK: ${colors.reset}${filePath} points to ${targetFullPath}, but target points to ${targetLinkInfo.target}`);
                                collectionIssues++;
                            }
                        }
                        ...
                        const matchingFiles = [...translationLinks.entries()]
                            .filter(([path, info]) => info.type === 'translationKey' && info.key === key);

                        if (matchingFiles.length < 2) {
                            console.log(`${colors.red}❌ ISOLATED KEY: ${colors.reset}${filePath} has translationKey "${key}" with no matching files`);
                            collectionIssues++;
                        }
                    }
                } catch (error) {
                    console.error(`${colors.red}Error verifying links for ${filePath}:${colors.reset}`, error);
                }
            }

            // Files without any translation links
            const filesWithoutLinks = [...addressBook.keys()].filter(file => !translationLinks.has(file));
            if (filesWithoutLinks.length > 0) {
                console.log(`\n${colors.yellow}Files without translation links:${colors.reset}`);
                filesWithoutLinks.forEach(file => {
                    console.log(`${colors.yellow}⚠️ NO TRANSLATION: ${colors.reset}${file}`);
                });
            ...
            if (collectionIssues === 0) {
                ...
                console.log(`${colors.yellow}ACTION NEEDED: Fix broken, missing, or non-reciprocal translation links.${colors.reset}`);
            }
        }
        ...
        console.error(`${colors.red}Error auditing content collections:${colors.reset}`, error);
    }
}
...
async function auditDataFiles() {
    ...
    console.log(`${colors.blue}Checking data files for complete bilingual data...${colors.reset}\n`);
    ...
    try {
        ...
        for (const dataFile of dataFiles) {
            ...
            try {
                ...
                console.log(`${colors.cyan}Checking ${dataFile.title} (${dataFile.name})${colors.reset}`);
                ...
                const objectRegex = /{\s*(?:[^{}]*?name(?:_es)?:\s*['"][^'"]*['"][^{}]*?)+\s*}/g;
                ...
                for (const objStr of objects) {
                    ...
                    if (nameMatch) {
                        ...
                        if (!nameEsMatch) {
                            console.log(`${colors.red}❌ MISSING SPANISH NAME: ${colors.reset}"${name}" is missing name_es property`);
                            missingProps++;
                        }

                        if (!slugMatch) {
                            console.log(`${colors.red}❌ MISSING SLUG: ${colors.reset}"${name}" is missing slug property`);
                            missingProps++;
                        }

                        if (!esSlugMatch) {
                            console.log(`${colors.red}❌ MISSING SPANISH SLUG: ${colors.reset}"${name}" is missing esSlug property`);
                            missingProps++;
                        }
                    }
                }
                ...
                if (missingProps === 0) {
                    console.log(`${colors.green}✓ All objects in ${dataFile.title} have complete bilingual properties!${colors.reset}\n`);
                } else {
                    console.log(`${colors.yellow}Found ${missingProps} missing properties in ${dataFile.title}.${colors.reset}`);
                    console.log(`${colors.yellow}ACTION NEEDED: Add missing name_es, slug, or esSlug properties.${colors.reset}\n`);
                    totalDataIssues += missingProps;
                }

            } catch (error) {
                if (error.code === 'ENOENT') {
                    console.log(`${colors.yellow}⚠️ File not found: ${dataFile.name}${colors.reset}\n`);
                } else {
                    console.error(`${colors.red}Error checking ${dataFile.name}:${colors.reset}`, error);
                }
            }
        }
        ...
        if (totalDataIssues === 0) {
            console.log(`${colors.green}All data files have complete bilingual information! ✓${colors.reset}`);
        } else {
            console.log(`${colors.yellow}Found ${totalDataIssues} total issues in data files.${colors.reset}`);
        }

    } catch (error) {
        console.error(`${colors.red}Error auditing data files:${colors.reset}`, error);
    }
}
...
async function findMarkdownFiles(dir) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            !excludePatterns.some(pattern => entry.name.includes(pattern.replace('**/', '').replace('/**', '')))) {
            files.push(fullPath);
        }
    }
...
function printSummary(staticIssues, contentIssues, dataIssues) {
    ...
    if (totalIssues === 0) {
        console.log(`${colors.green}✓ PERFECT SYMMETRY! Your site is fully bilingual and ready for sitemap generation.${colors.reset}`);
    } else {
        console.log(`${colors.yellow}Found ${totalIssues} total issues:${colors.reset}`);
        console.log(`${colors.yellow}- ${staticIssues} static page issues${colors.reset}`);
        console.log(`${colors.yellow}- ${contentIssues} content collection issues${colors.reset}`);
        console.log(`${colors.yellow}- ${dataIssues} data file issues${colors.reset}`);
