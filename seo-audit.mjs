...
const ASTRO_VAR_RE = (key) => new RegExp(`(?:export\\s+const\\s+|\\b)${key}\\s*[:=]\\s*["'\`](.*?)["'\`;]`, 'is'); // Added 's' flag
...
async function checkFile(filepath) {
    try {
        ...
        const h1Count = (content.match(H1_TAG_RE)?.length || 0) + (content.match(H1_MARKDOWN_RE)?.length || 0);
        if (h1Count > 1) {
        ...
        if (!title?.trim()) {
            ...
            report.addIssue('Title', `Title too long (${title.length}/${TITLE_LIMIT})`, title);
        }
        ...
        if (!metaDesc?.trim()) {
            ...
            if (metaDesc.length < DESC_MIN) {
                report.addIssue('Description', `Description too short (${metaDesc.length}/${DESC_MIN})`, metaDesc);
            }
            if (metaDesc.length > DESC_MAX) {
                report.addIssue('Description', `Description too long (${metaDesc.length}/${DESC_MAX})`, metaDesc);
            }
        }
...
async function runAudit() {
    ...
    const files = await glob(PATTERNS, { cwd: ROOT, absolute: true, ignore: IGNORE_PATTERNS });
